...
function entryFallbackReason(warehouseName?: string) {
  return warehouseName ? `Ingreso manual en ${warehouseName}` : "Ingreso manual en almacén";
}
...
function parseQuantityDraft(value: string | undefined) {
  if (!value?.trim()) {
    return undefined;
  }
...
export function InventoryWorkspace() {
  ...
  useEffect(() => {
    setQuantityDrafts(Object.fromEntries(rows.map((row) => [rowKey(row), ""])));
    setReasonDrafts({});
  }, [report?.generatedAt]);
  ...
  const selectedQuantityDraft = selectedKey ? quantityDrafts[selectedKey] : undefined;
  const selectedReasonDraft = selectedKey ? reasonDrafts[selectedKey] ?? "" : "";
  const selectedQuantityDelta = parseQuantityDraft(selectedQuantityDraft);
  const selectedNextStock =
    selectedRow && selectedQuantityDelta !== undefined ? selectedRow.stockOnHand + selectedQuantityDelta : undefined;
  ...
  const selectedBlockingMessage =
    !selectedRow
      ...
      : selectedRow.warehouseId === "unassigned"
        ...
        : selectedQuantityDelta === undefined
  ...
  const assignmentQuantity = parseQuantityDraft(assignment?.quantityDelta);
  ...
  async function handleSaveStock() {
    if (!selectedRow || !selectedKey || selectedQuantityDelta === undefined) {
      return;
    }
    ...
    try {
      const response = await adjustInventoryStock({
        ...
        mode: "increase",
        quantityDelta: selectedQuantityDelta,
  ...
  async function handleSaveAssignment() {
    if (!assignment || assignmentQuantity === undefined) {
      return;
    }
    ...
    try {
      const response = await adjustInventoryStock({
        ...
        mode: "increase",
        quantityDelta: assignmentQuantity,
              ...
              <DialogBody className="space-y-5">
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    placeholder={entryFallbackReason(warehouseLabel(selectedRow))}
                  />
                  <p className="mt-2 text-xs leading-5 text-black/50">
                    Si lo dejas vacío, se guardará con el motivo “{entryFallbackReason(warehouseLabel(selectedRow))}”.
                  </p>
                </div>
              ...
              <DialogBody className="space-y-5">
                ...
                {assignmentWarehouseOptions.length ? (
                  <>
                    ...
                    <div>
                      <label className="mb-1 block text-xs font-semibold uppercase tracking-[0.16em] text-black/45">
                        Stock actual en el nuevo almacén
                      </label>
                    ...
                    <div>
                      ...
                      <Input
                        ...
                        min={1}
                        step={1}
                        value={assignment.quantityDelta}
                        onChange={(event) =>
                          setAssignment((current) =>
                            current
                              ? {
                    ...
                    <div>
                      ...
                      <Textarea
                        ...
                        placeholder={entryFallbackReason(assignmentWarehouse?.name)}
                      />
                      <p className="mt-2 text-xs leading-5 text-black/50">
                        Si lo dejas vacío, se guardará con el motivo “{entryFallbackReason(assignmentWarehouse?.name)}”.
                      </p>
                    </div>
