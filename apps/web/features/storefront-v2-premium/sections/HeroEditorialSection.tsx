...
export function HeroEditorialSection({ hero, preview = false }: { hero: PremiumHeroContent; preview?: boolean }) {
  return (
    <section className="grid gap-6 xl:grid-cols-[1.02fr_0.98fr]">
      <div className="relative overflow-hidden rounded-[2.6rem] border border-[#112017]/8 bg-[linear-gradient(160deg,rgba(255,255,255,0.97)_0%,rgba(248,241,231,0.95)_55%,rgba(232,239,226,0.94)_100%)] px-7 py-8 shadow-[0_34px_90px_rgba(17,32,23,0.09)] md:px-10 md:py-10">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(144,160,127,0.22),transparent_34%),radial-gradient(circle_at_bottom_right,rgba(198,164,113,0.16),transparent_30%)]" />
        <div className="relative space-y-8">
          <div className="space-y-4">
            <div className="flex flex-wrap items-center gap-3">
              <Badge className="bg-[#112017] text-white">{hero.eyebrow}</Badge>
              {preview ? <span className="text-[11px] uppercase tracking-[0.28em] text-black/42">Vista controlada premium</span> : null}
            </div>
            <div className="space-y-4">
              <h1 className="max-w-4xl text-4xl font-semibold leading-[0.93] tracking-[-0.045em] text-[#112017] md:text-[4.55rem]">
                {hero.title}
              </h1>
          ...
          <div className="flex flex-wrap gap-2">
            {hero.productChips.map((chip) => (
              <span
                key={chip}
                className="inline-flex items-center rounded-full border border-[#112017]/10 bg-white/74 px-4 py-2 text-xs uppercase tracking-[0.22em] text-[#112017]/64 backdrop-blur"
              >
                {chip}
              </span>
            ))}
      ...
      <StorefrontV2PremiumMedia
        ...
        alt="Escena editorial premium de Huele Huele"
        priority
              ...
              {hero.notes.map((note) => {
                ...
                return (
                  ...
                  >
                    ...
                    <p className={`mt-2 text-sm font-semibold leading-6 ${dark ? "text-white" : "text-[#112017]"}`}>{note.title}</p>
                    <p className={`mt-2 text-sm leading-6 ${dark ? "text-white/72" : "text-black/58"}`}>{note.description}</p>
                  </div>
                );
              })}
            </div>
