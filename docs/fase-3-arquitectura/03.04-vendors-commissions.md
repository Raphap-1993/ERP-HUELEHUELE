...
| `payments` | cobro, revision manual y sync derivado tras pago | atribucion de vendedor, payout | `apps/api/src/modules/payments/*` (dependencia fuera de la superficie canonica de este slice) |
...
La verdad de estas senales no vive solo en el estado de la comision; se deriva
desde `commissions` y el runtime operativo de `worker`.
...
| cobro o revision manual del pago | fuera del slice canonico `002` | `payments` | dependencia operativa, no superficie canonica seller-first |
